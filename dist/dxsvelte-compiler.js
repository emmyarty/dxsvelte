...
#+DEBUG+#\r
print(output)\r
#+DEBUG+#`;import{join as q}from"path";import{existsSync as M,mkdirSync as hr,readdirSync as V,readFileSync as gr,rmSync as Sr,unlinkSync as B}from"fs";import{join as S,resolve as vr}from"path";import*as T from"url";var E="__svcache__",yr="python",l=vr(process.cwd()),wr=T.fileURLToPath(import.meta.url),_r=T.fileURLToPath(new URL(".",import.meta.url)),i=S(l,E),br=M(S(l,"manage.py"));br||(console.error("This script must be run from the Django project's root directory. Exiting."),process.exit(1));console.log(`${l} is a Django project directory. Continuing.`);var L=W(),G=W(),h=S(l,G);function W(){let r=/os\.environ\.setdefault\(\s*(['\"`])DJANGO_SETTINGS_MODULE\1\s*,\s*\s*(['\"`])(.+?)\2\s*\)/,e=gr(S(l,"manage.py"),"utf8").match(r)??[],n=e?.length>3?e[3]:"";if(n==="")throw new Error("Could not extract settings from manage.py. Exiting.");return n.split(".")[0]}function k(){M(i)?V(i).forEach(t=>{let e=S(i,t);B(e)}):hr(i,{recursive:!0})}function $(){V(i).forEach(t=>{let e=S(i,t);B(e)}),Sr(i,{recursive:!0,force:!0}),console.log(`Cleaned ${E} artefacts.`)}var U={app_name:L,cache:E,pythonCmd:yr,__basedir:l,__filename:wr,__dirname:_r,__cache:i,__main:G,__maindir:h,prepareSvCache:k,cleanSvCache:$};import{writeFileSync as Rr}from"fs";function xr(r){return r&&r.replace(/\\/g,"/")}function Pr(r,t){if(t.length<2)throw new Error(`${t} is not a valid component file.`);let e=`${t.slice(1)}.svelte`;return xr(q(l,r,"views",e))}function jr(r){return r.length<2?r:r.charAt(1).toUpperCase()+r.slice(2).replace("$","")}function Or(r){if(r.length===0)return r;let t=r.charAt(0).toUpperCase();return r.length===1?t:t+r.slice(1)}function X(...r){return"/"+r.join("/").replace(/\/\/+/g,"/").replace(/^\/|\/$/g,"")}function K(r){let t=[],e=(n,o)=>{if(o.type==="resolver"){if(!Array.isArray(o.url_patterns))return null;o.url_patterns.map(a=>{e({...o},a)})}if(o.type==="pattern"){if((!n||!n.app_path)&&(n={app_path:L,type:"resolver",prefix:""}),!n||!n.app_path||typeof o.pattern!="string"||!o.name||o.name[0]!=="$")return null;let a={app:n.app_path,path:n.prefix?X(n.prefix,o.pattern):X(o.pattern),view:o.name??null,static:o.static_view,component:o.name?Or(n.app_path)+jr(o.name):null,filename:o.name&&n.app_path?Pr(n.app_path,o.name):null};t.push(a)}};return Array.isArray(r)&&(r.map(n=>e(null,n)),Rr(q(i,"debugRouter.json"),JSON.stringify(t,null,2))),t}import{execSync as Tr}from"child_process";import{readFileSync as Er,writeFileSync as Lr}from"fs";import Y from"path";import{readFileSync as Yr}from"fs";import{resolve as Q}from"path";import{compile as Cr}from"svelte/compiler";function d(r,t){return Object.keys(r).forEach(e=>{let n=new RegExp(`{{${e}}}`,"g"),o=new RegExp(`{{!${e}}}`,"g");t=t.replace(n,r[e]),t=t.replace(o,`{{${e}}}`)}),t}function Dr(r,t){let e=Q(r);return t.find(n=>Q(n.sourcefile)===e)??null}function z(r=[],t){return{name:"virtual-file-injector",setup(e){e.onResolve({filter:/\.vf\.[\S]+$/},async n=>({path:n.path})),e.onLoad({filter:/\.vf\.[\S]+$/},async n=>{let o=Dr(n.path,r),a=o.contents,p=o.loader;if(p==="svelte"){let f=Cr(a,t)?.js?.code??null;if(!f)throw new Error("Compilation Failed for: "+n.path);return{contents:f,loader:"js"}}return{contents:a,loader:p}})}}}var{__cache:Z,app_name:kr,cache:$r}=U;function rr(){console.log("Getting router...");function r(a,p){let u=new RegExp(`#\\+${p}\\+#([\\s\\S]*?)#\\+${p}\\+#`,"gm"),f=new RegExp("#\\+([\\S]*?)\\+#","gm");return a=a.replace(u,""),a=a.replace(f,""),a}let t;t=process.env.NODE_ENV==="debug"?r(D,"OPERATIONAL"):r(D,"DEBUG"),t=d({app_name:kr,cache:$r},t);function e(){let a=Y.join(Z,"tmp"),p=Y.join(Z,"routerResolver.json");try{Lr(a,t),Tr(`${U.pythonCmd} ${a}`);let u=Er(p,"utf8");return JSON.parse(u)}catch(u){throw console.error(u),new Error("Could Not Load Django Router Object")}}let n=e();return K(n)}var tr="<slot/>";var er=`<script>\r
...
\r
export default { ViewState, getCsrfTokenHeader, FormSetup }`;import{existsSync as Ur}from"fs";function c(r){return r&&r.replace(/\\/g,"/")}import{join as g}from"path";function lr(r){let t=[];function e(s,O){let R=s.split("."),C=R[R.length-1],mr={contents:O,sourcefile:s,loader:C};t.push(mr)}let n=c(g(h,"layout.svelte"));Ur(n)||(n=c(g(h,"layout.vf.svelte")),e(n,tr));let o=c(g(i,"root.vf.svelte")),a=`import Layout from '${n}'`,p=s=>`import ${s.component} from '${s.filename}'`,u=s=>`{#each trigger as instance}{#if satisfiedStorePath(currentView) === '${s.path}'}<${s.component}></${s.component}>{/if}{/each}`,f=r.map(s=>p(s)),P=r.map(s=>u(s)),F=f.join(`
`),j=P.join(`
`),m=JSON.stringify(r.map(s=>({path:s.path,static:s.static}))),v=c(g(h,"router.vf.ts")),J=d({strRouterArrayOfObjects:m},ar);e(v,J);let b=d({router:v,layoutImportStatement:a,svelteComponentImports:F,svelteComponentsIfs:j},er),N=s=>{let O={path:s.path,fnameRouter:v},R=d(O,sr),C=`${s.filename}.page.vf.ts`;e(C,R)};r.map(s=>N(s));let y=c(g(i,"common.vf.ts")),w=d({fnameRouter:v},ir);e(y,w);let fr=d({App:o},nr),dr=d({App:o},or),I=`${c(g(i,"ssr.vf.js"))}`,H=`${c(g(i,"csr.vf.js"))}`;return e(o,b),e(I,fr),e(H,dr),{vfLoaders:t,entrypointSSRPath:I,entrypointCSRPath:H}}import Ar from"esbuild";import Fr from"esbuild-svelte";import{join as _}from"path";import{mkdirSync as Jr,existsSync as Nr}from"fs";import{pathToFileURL as Ir}from"url";function Hr(){return{name:"svelte-data-resolver",setup(r){r.onResolve({filter:/^@page$/},t=>({path:`${c(t.importer)}.page.vf.ts`})),r.onResolve({filter:/^@common$/},()=>({path:c(_(i,"common.vf.ts"))}))}}}async function A(r,t,e){console.log("Compiling...",e,r);let a=e==="ssr"?{generate:"ssr",dev:!1,hydratable:!0,format:"esm",css:!0}:{generate:"dom",dev:!1,format:"esm"};try{Jr(_(l,"static"))}catch{}let p=e==="ssr"?_(h,"svelte.ssr.js"):_(l,"static","svelte.csr.js");async function u(m){let b=[m+".js",m+".mjs",m+".cjs"].map(w=>_(l,w)).find(w=>Nr(w));if(!b)return[];let y=await import(Ir(b).toString());if(typeof y<"u")return console.log("Loaded "+m),console.log(y),y}let f=e==="ssr"?{preprocess:[]}:await u("svelte.config"),P=f?.preprocess?f.preprocess:[],F=!0,j=[z(t,a),Hr(),Fr({preprocess:P,compilerOptions:a})];return Ar.build({entryPoints:[r],mainFields:["svelte","browser","module","main"],bundle:!0,outfile:p,format:"esm",plugins:j}).catch(()=>{console.error(e==="csr"?"CSR Application Build Failed. Exiting.":"SSR Application Build Failed. Exiting."),process.exit(1)})}import cr from"fs";import{join as Mr}from"path";function pr(){let r=c(Mr(l,"manage.py"));cr.writeFileSync(r,cr.readFileSync(r))}k();var ur=rr(),x=lr(ur);console.dir(ur);await A(x.entrypointSSRPath,x.vfLoaders,"ssr");await A(x.entrypointCSRPath,x.vfLoaders,"csr");$();pr();
