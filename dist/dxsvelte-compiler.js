...
#+DEBUG+#\r
print(output)\r
#+DEBUG+#`;import{join as K}from"path";import{existsSync as V,mkdirSync as hr,readdirSync as B,readFileSync as gr,rmSync as Sr,unlinkSync as G}from"fs";import{join as S,resolve as vr}from"path";import*as L from"url";var T="__svcache__",yr="python",l=vr(process.cwd()),wr=L.fileURLToPath(import.meta.url),_r=L.fileURLToPath(new URL(".",import.meta.url)),i=S(l,T),Rr=V(S(l,"manage.py"));Rr||(console.error("This script must be run from the Django project's root directory. Exiting."),process.exit(1));console.log(`${l} is a Django project directory. Continuing.`);var E=X(),W=X(),h=S(l,W);function X(){let r=/os\.environ\.setdefault\(\s*(['\"`])DJANGO_SETTINGS_MODULE\1\s*,\s*\s*(['\"`])(.+?)\2\s*\)/,e=gr(S(l,"manage.py"),"utf8").match(r)??[],n=e?.length>3?e[3]:"";if(n==="")throw new Error("Could not extract settings from manage.py. Exiting.");return n.split(".")[0]}function k(){V(i)?B(i).forEach(t=>{let e=S(i,t);G(e)}):hr(i,{recursive:!0})}function $(){B(i).forEach(t=>{let e=S(i,t);G(e)}),Sr(i,{recursive:!0,force:!0}),console.log(`Cleaned ${T} artefacts.`)}var U={app_name:E,cache:T,pythonCmd:yr,__basedir:l,__filename:wr,__dirname:_r,__cache:i,__main:W,__maindir:h,prepareSvCache:k,cleanSvCache:$};import{writeFileSync as br}from"fs";function xr(r){return r&&r.replace(/\\/g,"/")}function Pr(r,t){if(t.length<2)throw new Error(`${t} is not a valid component file.`);let e=`${t.slice(1)}.svelte`;return xr(K(l,r,"views",e))}function jr(r){return r.length<2?r:r.charAt(1).toUpperCase()+r.slice(2).replace("$","")}function Or(r){if(r.length===0)return r;let t=r.charAt(0).toUpperCase();return r.length===1?t:t+r.slice(1)}function q(...r){return"/"+r.join("/").replace(/\/\/+/g,"/").replace(/^\/|\/$/g,"")}function Q(r){let t=[],e=(n,o)=>{if(o.type==="resolver"){if(!Array.isArray(o.url_patterns))return null;o.url_patterns.map(a=>{e({...o},a)})}if(o.type==="pattern"){if((!n||!n.app_path)&&(n={app_path:E,type:"resolver",prefix:""}),!n||!n.app_path||typeof o.pattern!="string"||!o.name||o.name[0]!=="$")return null;let a={app:n.app_path,path:n.prefix?q(n.prefix,o.pattern):q(o.pattern),view:o.name??null,static:o.static_view,component:o.name?Or(n.app_path)+jr(o.name):null,filename:o.name&&n.app_path?Pr(n.app_path,o.name):null};t.push(a)}};return Array.isArray(r)&&(r.map(n=>e(null,n)),br(K(i,"debugRouter.json"),JSON.stringify(t,null,2))),t}import{execSync as Lr}from"child_process";import{readFileSync as Tr,writeFileSync as Er}from"fs";import Z from"path";import{readFileSync as Yr}from"fs";import{resolve as z}from"path";import{compile as Cr}from"svelte/compiler";function m(r,t){return Object.keys(r).forEach(e=>{let n=new RegExp(`{{${e}}}`,"g"),o=new RegExp(`{{!${e}}}`,"g");t=t.replace(n,r[e]),t=t.replace(o,`{{${e}}}`)}),t}function Dr(r,t){let e=z(r);return t.find(n=>z(n.sourcefile)===e)??null}function Y(r=[],t){return{name:"virtual-file-injector",setup(e){e.onResolve({filter:/\.vf\.[\S]+$/},async n=>({path:n.path})),e.onLoad({filter:/\.vf\.[\S]+$/},async n=>{let o=Dr(n.path,r),a=o.contents,p=o.loader;if(p==="svelte"){let d=Cr(a,t)?.js?.code??null;if(!d)throw new Error("Compilation Failed for: "+n.path);return{contents:d,loader:"js"}}return{contents:a,loader:p}})}}}var{__cache:rr,app_name:kr,cache:$r}=U;function tr(){console.log("Getting router...");function r(a,p){let f=new RegExp(`#\\+${p}\\+#([\\s\\S]*?)#\\+${p}\\+#`,"gm"),d=new RegExp("#\\+([\\S]*?)\\+#","gm");return a=a.replace(f,""),a=a.replace(d,""),a}let t;t=process.env.NODE_ENV==="debug"?r(D,"OPERATIONAL"):r(D,"DEBUG"),t=m({app_name:kr,cache:$r},t);function e(){let a=Z.join(rr,"tmp"),p=Z.join(rr,"routerResolver.json");try{Er(a,t),Lr(`${U.pythonCmd} ${a}`);let f=Tr(p,"utf8");return JSON.parse(f)}catch(f){throw console.error(f),new Error("Could Not Load Django Router Object")}}let n=e();return Q(n)}var er="<slot/>";var nr=`<script>\r
...
`;var or=`// Before evaluating this script, we must set the SSRPATH, SSRJSON, and CSRFTOKEN variables within the context object\r
\r
...
const result = JSON.stringify({ head, html: htmlPreload, css });`;var ar=`// Import compiled CSR Svelte app.\r
import App from '{{App}}';\r
\r
...
document.body.replaceWith(container);`;var sr=`import { Writable, writable } from "svelte/store";\r
\r
declare global {\r
  interface Window {\r
    mode?: string;\r
...
`;var ir=`// This is a file which gets generated per module and in the same directory.\r
...
\r
export default { ServerSideProps }`;var lr=`// This is a file which gets generated per module and in the same directory.\r
// It ensures that objects being imported are 'localised' as appropriate.\r
\r
//@ts-ignore\r
...
\r
export default { ViewState, getCsrfTokenHeader, FormSetup }`;import{existsSync as Ur}from"fs";function c(r){return r&&r.replace(/\\/g,"/")}import{join as g}from"path";function cr(r){let t=[];function e(s,O){let b=s.split("."),C=b[b.length-1],mr={contents:O,sourcefile:s,loader:C};t.push(mr)}let n=c(g(h,"layout.svelte"));Ur(n)||(n=c(g(h,"layout.vf.svelte")),e(n,er));let o=c(g(i,"root.vf.svelte")),a=`import Layout from '${n}'`,p=s=>`import ${s.component} from '${s.filename}'`,f=s=>`{#each trigger as instance}{#if satisfiedStorePath(currentView) === '${s.path}'}<${s.component}></${s.component}>{/if}{/each}`,d=r.map(s=>p(s)),_=r.map(s=>f(s)),P=d.join(`
`),A=_.join(`
`),j=JSON.stringify(r.map(s=>({path:s.path,static:s.static}))),u=c(g(h,"router.vf.ts")),J=m({strRouterArrayOfObjects:j},sr);e(u,J);let N=m({router:u,layoutImportStatement:a,svelteComponentImports:P,svelteComponentsIfs:A},nr),R=s=>{let O={path:s.path,fnameRouter:u},b=m(O,ir),C=`${s.filename}.page.vf.ts`;e(C,b)};r.map(s=>R(s));let I=c(g(i,"common.vf.ts")),v=m({fnameRouter:u},lr);e(I,v);let y=m({App:o},or),dr=m({App:o},ar),H=`${c(g(i,"ssr.vf.js"))}`,M=`${c(g(i,"csr.vf.js"))}`;return e(o,N),e(H,y),e(M,dr),{vfLoaders:t,entrypointSSRPath:H,entrypointCSRPath:M}}import Fr from"esbuild";import Ar from"esbuild-svelte";import{join as w}from"path";import{mkdirSync as Jr,existsSync as Nr}from"fs";import{pathToFileURL as Ir}from"url";function Hr(){return{name:"svelte-data-resolver",setup(r){r.onResolve({filter:/^@page$/},t=>({path:`${c(t.importer)}.page.vf.ts`})),r.onResolve({filter:/^@common$/},()=>({path:c(w(i,"common.vf.ts"))}))}}}async function F(r,t,e){console.log("Compiling...",e,r);let a=e==="ssr"?{generate:"ssr",dev:!1,hydratable:!0,format:"esm",css:!0}:{generate:"dom",dev:!1,format:"esm"},p=e==="csr";try{Jr(w(l,"static"))}catch{}let f=e==="ssr"?w(h,"svelte.ssr.js"):w(l,"static","svelte.csr.js");async function d(u){let R=[u+".js",u+".mjs",u+".cjs"].map(y=>w(l,y)).find(y=>Nr(y));if(!R)return[];let v=await import(Ir(R).toString());if(typeof v<"u")return console.log("Loaded "+u),console.log(v),v}let _=e==="ssr"?{preprocess:[]}:await d("svelte.config"),P=_?.preprocess?_.preprocess:[],A=!0,j=[Y(t,a),Hr(),Ar({preprocess:P,compilerOptions:a})];return Fr.build({entryPoints:[r],mainFields:["svelte","browser","module","main"],bundle:!0,outfile:f,format:"esm",plugins:j,minify:p}).catch(()=>{console.error(e==="csr"?"CSR Application Build Failed. Exiting.":"SSR Application Build Failed. Exiting."),process.exit(1)})}import pr from"fs";import{join as Mr}from"path";function ur(){let r=c(Mr(l,"manage.py"));pr.writeFileSync(r,pr.readFileSync(r))}k();var fr=tr(),x=cr(fr);console.dir(fr);await F(x.entrypointSSRPath,x.vfLoaders,"ssr");await F(x.entrypointCSRPath,x.vfLoaders,"csr");$();ur();
