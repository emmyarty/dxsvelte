...
\r
export default { ViewState }`;import{existsSync as Ur}from"fs";function c(r){return r&&r.replace(/\\/g,"/")}import{join as g}from"path";function lr(r){let t=[];function e(s,O){let R=s.split("."),C=R[R.length-1],mr={contents:O,sourcefile:s,loader:C};t.push(mr)}let n=c(g(h,"layout.svelte"));Ur(n)||(n=c(g(h,"layout.vf.svelte")),e(n,tr));let o=c(g(i,"root.vf.svelte")),a=`import Layout from '${n}'`,p=s=>`import ${s.component} from '${s.filename}'`,u=s=>`{#each trigger as instance}{#if satisfiedStorePath(currentView) === '${s.path}'}<${s.component}></${s.component}>{/if}{/each}`,f=r.map(s=>p(s)),P=r.map(s=>u(s)),J=f.join(`
`),j=P.join(`
`),m=JSON.stringify(r.map(s=>({path:s.path,static:s.static}))),v=c(g(h,"router.vf.ts")),N=d({strRouterArrayOfObjects:m},ar);e(v,N);let b=d({router:v,layoutImportStatement:a,svelteComponentImports:J,svelteComponentsIfs:j},er),F=s=>{let O={path:s.path,fnameRouter:v},R=d(O,sr),C=`${s.filename}.page.vf.ts`;e(C,R)};r.map(s=>F(s));let y=c(g(i,"common.vf.ts")),w=d({fnameRouter:v},ir);e(y,w);let fr=d({App:o},nr),dr=d({App:o},or),I=`${c(g(i,"ssr.vf.js"))}`,H=`${c(g(i,"csr.vf.js"))}`;return e(o,b),e(I,fr),e(H,dr),{vfLoaders:t,entrypointSSRPath:I,entrypointCSRPath:H}}import Ar from"esbuild";import Jr from"esbuild-svelte";import{join as _}from"path";import{mkdirSync as Nr,existsSync as Fr}from"fs";import{pathToFileURL as Ir}from"url";function Hr(){return{name:"svelte-data-resolver",setup(r){r.onResolve({filter:/^@page$/},t=>({path:`${c(t.importer)}.page.vf.ts`})),r.onResolve({filter:/^@common$/},()=>({path:c(_(i,"common.vf.ts"))}))}}}async function A(r,t,e){console.log("Compiling...",e,r);let a=e==="ssr"?{generate:"ssr",dev:!1,hydratable:!0,format:"esm",css:!0}:{generate:"dom",dev:!1,format:"esm"};try{Nr(_(l,"static"))}catch{}let p=e==="ssr"?_(h,"svelte.ssr.js"):_(l,"static","svelte.csr.js");async function u(m){let b=[m+".js",m+".mjs",m+".cjs"].map(w=>_(l,w)).find(w=>Fr(w));if(!b)return[];let y=await import(Ir(b).toString());if(typeof y<"u")return console.log("Loaded "+m),console.log(y),y}let f=e==="ssr"?{preprocess:[]}:await u("svelte.config"),P=f?.preprocess?f.preprocess:[],J=!0,j=[z(t,a),Hr(),Jr({preprocess:P,compilerOptions:a})];return Ar.build({entryPoints:[r],mainFields:["svelte","browser","module","main"],bundle:!0,outfile:p,format:"esm",plugins:j}).catch(()=>{console.error(e==="csr"?"CSR Application Build Failed. Exiting.":"SSR Application Build Failed. Exiting."),process.exit(1)})}import cr from"fs";import{join as Mr}from"path";function pr(){let r=c(Mr(l,"manage.py"));cr.writeFileSync(r,cr.readFileSync(r))}k();var ur=rr(),x=lr(ur);console.dir(ur);await A(x.entrypointSSRPath,x.vfLoaders,"ssr");await A(x.entrypointCSRPath,x.vfLoaders,"csr");$();pr();
