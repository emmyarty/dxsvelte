...
import routesStatic from "@dxsvelte:routesStatic"

export const routes = routesStatic as RoutePathStatic[];
  ...
  async fetch(target:string|null = null) {
    try {
      ...
      if (resultRaw.type == 'opaqueredirect'){ // <- Catch and handle regular Django's redirect
        ...
        return location.href = trimmedTarget ? trimmedTarget : "";
      }
      ...
      return location.href = onlyPath(target ?? this.storePath);// <- Catch and handle errors
